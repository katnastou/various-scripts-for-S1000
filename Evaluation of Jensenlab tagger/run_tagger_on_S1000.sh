...
ls -1 S1000-jensenlab-tagger/entire-corpus/test | perl -pi -e "s/\.txt$//g" > s1000_test_pmids.list
...
scp -r knastou@cyan.jensenlab.org:/jensenlab/home/knastou/brat/tools/annalign.py .
...
cp -r S1000-extension-tagger-with-dictionary-extensions/test-set-filtered{,-aligned}

for f in S1000-extension-tagger-with-dictionary-extensions/test-set-filtered/*.ann; do python3 annalign.py $f ${f%.ann}.txt S800-extension/test-set/$(basename $f .ann).txt > S1000-extension-tagger-with-dictionary-extensions/test-set-filtered-aligned/$(basename $f); done
